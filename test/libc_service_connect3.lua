...
if _CONTEXT ~= 'main' then
    ...
    s:connect(ip.address.new('::1%2'), 1)
...
slave.connect_inet6 = [[
local real_connect, fd, addr, port, scope_id = ...
local res, errno, fd2 = real_connect(fd, addr, port, scope_id)
if fd2 then
    C.dup2(fd2, fd)
    C.close(fd2)
end
...
spawn(function() pcall(function()
    while true do
        master:receive()
        if master.function_ == 'connect_inet6' then
            local addr, port = master:arguments()
            if addr == ip.address.new('::1%2') and port == 1 then
