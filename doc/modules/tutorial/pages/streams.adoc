...
write_all -> stream : write_some(buf1:sub(N1))

stream --> write_all : buf2/OK
write_all --> writer2 : buf2/OK
deactivate write_all

stream --> write_all : buf1:sub(N1)/OK
...
`buf1:sub(N1))`. The end result will be a stream where the second message is
