...
namespaces are involved. It'll just be passed along to the new process.
...
`subprocess.new_uts: boolean = false`:: Whether to create the process within a
new UTS namespace.

`subprocess.new_ipc: boolean = false`:: Whether to create the process within a
new IPC namespace.

`subprocess.new_pid: boolean = false`::
...
`subprocess.new_user: boolean = false`::
...
`subprocess.new_net: boolean = false`:: Whether to create the process within a
new net namespace.

`subprocess.new_mount: boolean = false`:: Whether to create the process within a
new mount namespace.
...
`subprocess.init.fd: file_descriptor`:: A file descriptor that will be sent to
the `init.script`. The script can access this fd through the variable `fdarg`
